 ...
 */
function getWorldFrequency(value) {
  const { note, freq, scale = ScaleType.JAPANESE_YO } = value;
...
export function registerWorldSynths() {
  ...
  registerSound(
    ...
    (t, value, onended) => {
      ...
      const frequency = getWorldFrequency({ ...value, scale });
  ...
  scaleShortcuts.forEach(([name, scaleType]) => {
    registerSound(
      name,
      (t, value, onended) => {
        // Use the main world synth with preset scale
        const worldSound = registerSound.get('world');
        return worldSound(t, { ...value, scale: scaleType }, onended);
      },
      { type: 'synth', prebake: true }
    );
  });
